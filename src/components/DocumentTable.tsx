import React, { useState, useEffect } from 'react'
import { Table, Input, Button, Typography, App } from 'antd'
import { PlusOutlined, EditOutlined, SaveOutlined, CloseOutlined, DeleteOutlined } from '@ant-design/icons'

const { Title } = Typography

interface DocumentItem {
  key: string
  id: number
  name: string
}

const DocumentTable: React.FC = () => {
  const { message } = App.useApp()
  const [documents, setDocuments] = useState<DocumentItem[]>([])
  const [editingKey, setEditingKey] = useState<string>('')
  const [editingValue, setEditingValue] = useState<string>('')
  const [editingColumnHeader, setEditingColumnHeader] = useState<boolean>(false)
  const [columnHeaderValue, setColumnHeaderValue] = useState<string>('')
  const [columnTitle, setColumnTitle] = useState<string>('Наименование документа')

  // Загрузка документов из localStorage
  useEffect(() => {
    const savedDocuments = localStorage.getItem('documents')
    if (savedDocuments) {
      try {
        const parsedDocuments = JSON.parse(savedDocuments)
        setDocuments(parsedDocuments)
      } catch (error) {
        console.error('Ошибка загрузки документов:', error)
      }
    } else {
      // Начальные данные
      const initialDocuments = [
        { key: '1', id: 1, name: 'Документ 1' },
        { key: '2', id: 2, name: 'Документ 2' },
        { key: '3', id: 3, name: 'Документ 3' }
      ]
      setDocuments(initialDocuments)
      saveDocuments(initialDocuments)
    }

    // Загрузка заголовка столбца
    const savedColumnTitle = localStorage.getItem('documentsColumnTitle')
    if (savedColumnTitle) {
      setColumnTitle(savedColumnTitle)
    }
  }, [])

  // Сохранение документов в localStorage
  const saveDocuments = (documentsToSave: DocumentItem[]) => {
    try {
      localStorage.setItem('documents', JSON.stringify(documentsToSave))
    } catch (error) {
      console.error('Ошибка сохранения документов:', error)
    }
  }

  // Начать редактирование
  const startEdit = (key: string, name: string) => {
    setEditingKey(key)
    setEditingValue(name)
  }

  // Сохранить изменения
  const saveEdit = () => {
    if (editingValue.trim()) {
      const updatedDocuments = documents.map(document => 
        document.key === editingKey 
          ? { ...document, name: editingValue.trim() }
          : document
      )
      setDocuments(updatedDocuments)
      saveDocuments(updatedDocuments)
      setEditingKey('')
      setEditingValue('')
      message.success('Документ обновлен')
    }
  }

  // Отменить редактирование
  const cancelEdit = () => {
    setEditingKey('')
    setEditingValue('')
  }

  // Добавить новый документ
  const addDocument = () => {
    const newId = Math.max(...documents.map(document => document.id), 0) + 1
    const newDocument: DocumentItem = {
      key: newId.toString(),
      id: newId,
      name: `Документ ${newId}`
    }
    const updatedDocuments = [...documents, newDocument]
    setDocuments(updatedDocuments)
    saveDocuments(updatedDocuments)
    message.success('Новый документ добавлен')
  }

  // Удалить документ
  const deleteDocument = (key: string) => {
    const updatedDocuments = documents.filter(document => document.key !== key)
    setDocuments(updatedDocuments)
    saveDocuments(updatedDocuments)
    message.success('Документ удален')
  }

  const startEditColumnHeader = () => {
    setEditingColumnHeader(true)
    setColumnHeaderValue(columnTitle)
  }

  const saveColumnHeader = () => {
    if (columnHeaderValue.trim()) {
      setColumnTitle(columnHeaderValue.trim())
      localStorage.setItem('documentsColumnTitle', columnHeaderValue.trim())
      setEditingColumnHeader(false)
      setColumnHeaderValue('')
      message.success('Заголовок столбца обновлен')
    }
  }

  const cancelEditColumnHeader = () => {
    setEditingColumnHeader(false)
    setColumnHeaderValue('')
  }

  const columns = [
    {
      title: '№ п/п',
      dataIndex: 'id',
      key: 'id',
      width: 100,
      render: (id: number, record: DocumentItem, index: number) => (
        <div className="text-center font-medium">{index + 1}</div>
      )
    },
    {
      title: editingColumnHeader ? (
        <div className="flex items-center gap-2">
          <Input
            value={columnHeaderValue}
            onChange={(e) => setColumnHeaderValue(e.target.value)}
            onPressEnter={saveColumnHeader}
            autoFocus
            size="small"
          />
          <Button
            type="link"
            size="small"
            icon={<SaveOutlined />}
            onClick={saveColumnHeader}
            className="text-green-600"
          />
          <Button
            type="link"
            size="small"
            icon={<CloseOutlined />}
            onClick={cancelEditColumnHeader}
            className="text-red-600"
          />
        </div>
      ) : (
        <div className="flex items-center justify-between group">
          <span>{columnTitle}</span>
          <Button
            type="link"
            size="small"
            icon={<EditOutlined />}
            onClick={startEditColumnHeader}
            className="opacity-0 group-hover:opacity-100 transition-opacity"
          />
        </div>
      ),
      dataIndex: 'name',
      key: 'name',
      render: (name: string, record: DocumentItem) => {
        const isEditing = editingKey === record.key

        if (isEditing) {
          return (
            <div className="flex items-center gap-2">
              <Input
                value={editingValue}
                onChange={(e) => setEditingValue(e.target.value)}
                onPressEnter={saveEdit}
                autoFocus
              />
              <Button
                type="link"
                size="small"
                icon={<SaveOutlined />}
                onClick={saveEdit}
                className="text-green-600"
              />
              <Button
                type="link"
                size="small"
                icon={<CloseOutlined />}
                onClick={cancelEdit}
                className="text-red-600"
              />
            </div>
          )
        }

        return (
          <div className="flex items-center justify-between group">
            <span>{name}</span>
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button
                type="link"
                size="small"
                icon={<EditOutlined />}
                onClick={() => startEdit(record.key, name)}
              />
              <Button
                type="link"
                size="small"
                icon={<DeleteOutlined />}
                onClick={() => deleteDocument(record.key)}
                className="text-red-600"
              />
            </div>
          </div>
        )
      }
    }
  ]

  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <Title level={3}>Справочник документов</Title>
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={addDocument}
        >
          Добавить документ
        </Button>
      </div>

      <Table
        columns={columns}
        dataSource={documents}
        pagination={false}
        bordered
        size="middle"
        className="document-table"
      />
    </div>
  )
}

export default DocumentTable